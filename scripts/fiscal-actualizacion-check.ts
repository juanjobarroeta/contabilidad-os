...
import { evaluarCoberturaFiscal } from "../src/lib/fiscal/cobertura-datos";
...
check("inpc '2025-12' (serie canónica) = 143.042", inpcPeriodo("2025-12") === 143.042, `got ${inpcPeriodo("2025-12")}`);
check("inpc '2026-08' (INEGI 586/26) = 145.462", inpcPeriodo("2026-08") === 145.462, `got ${inpcPeriodo("2026-08")}`);
...
const coberturaFiscal = evaluarCoberturaFiscal();
const estadoInpc = coberturaFiscal.datasets.find((d) => d.clave === "INPC");
const cubreUltimoPublicado = Boolean(
  estadoInpc?.ultimoCargado && estadoInpc.ultimoCargado >= estadoInpc.ultimoEsperado,
);
console.log(
  `    cobertura ./inpc: ${cob?.year}-${String(cob?.month).padStart(2, "0")} · ` +
    `último publicado esperado: ${estadoInpc?.ultimoEsperado ?? "desconocido"}`,
);
if (strict) {
  check(
    "serie cubre el último periodo publicado",
    cubreUltimoPublicado,
    `cargado ${estadoInpc?.ultimoCargado ?? "ninguno"}; esperado ${estadoInpc?.ultimoEsperado ?? "desconocido"}`,
  );
} else if (!cubreUltimoPublicado) {
  console.log(
    `  ⚠ serie atrasada: cargado ${estadoInpc?.ultimoCargado ?? "ninguno"}; ` +
      `esperado ${estadoInpc?.ultimoEsperado ?? "desconocido"} (no falla sin --strict)`,
  );
}
