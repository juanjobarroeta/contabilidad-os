/**
 * One-shot: enable the RESTAURANTE module on a Company.
 *
 * Usage:
 ...
 *
 * Example:
 *   node scripts/enable-restaurante-module.mjs BRC250101ABC
 *
 * Idempotent: upserts the CompanyModule row. Prints the Company + all its
 * current modules so you can verify the state before re-logging into restauranteos.
 *
...
async function main() {
  ...
  if (!rfc) {
    console.error("Usage: node scripts/enable-restaurante-module.mjs <rfc>");
    process.exit(1);
  }
  ...
  // Enable RESTAURANTE
  await prisma.companyModule.upsert({
    where: { companyId_modulo: { companyId: company.id, modulo: "RESTAURANTE" } },
    create: { companyId: company.id, modulo: "RESTAURANTE" },
    update: { habilitado: true },
  });
  ...
  console.log(
    `Next: log out + log back in on restauranteos.vercel.app so AuthContext re-fetches the companies list.`
  );
}
