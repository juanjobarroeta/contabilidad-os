/**
 * One-shot: enable the PURIFICADORA module on a Company.
 *
 * Usage:
 *   set -a; . ./.env.local; set +a
 *   node scripts/enable-purificadora-module.mjs <rfc>
 *
 * Example:
 *   node scripts/enable-purificadora-module.mjs PUR250101ABC
 *
 * Idempotent: upserts the CompanyModule row. Prints the Company + all its
 * current modules so you can verify the state before re-logging into the
 * purificadora satellite.
 */
...
async function main() {
  ...
  if (!rfc) {
    console.error("Usage: node scripts/enable-purificadora-module.mjs <rfc>");
    process.exit(1);
  }
  ...
  // Enable PURIFICADORA
  await prisma.companyModule.upsert({
    where: { companyId_modulo: { companyId: company.id, modulo: "PURIFICADORA" } },
    create: { companyId: company.id, modulo: "PURIFICADORA" },
    update: { habilitado: true },
  });
  ...
  console.log(
    `Next: log out + log back in on the purificadora satellite so AuthContext re-fetches the companies list.`
  );
}
