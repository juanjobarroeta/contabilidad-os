...
| V-1 Bóveda: chokepoint + auditoría de lecturas | **En PR** (2026-08-17): `src/lib/vault.ts` es el único lugar que descifra (candado en CI vía `vault-enforcement.test.ts`, verificado rojo/verde). Todo descifrado declara tipo+propósito contra allowlist y deja `credencial.uso` en la bitácora (fiel/csd uno a uno; pac-call agrupado por hora). `PacProvider` ahora recibe `PacCredencial` (llave cifrada + companyId). Borrado de credenciales en baja.ts ya deja bitácora. Cero cambios de criptografía. |
