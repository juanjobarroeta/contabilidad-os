...
| P0-5 Security headers | **En PR** (2026-08-17): HSTS, nosniff, X-Frame-Options DENY, Referrer-Policy, Permissions-Policy en toda respuesta (lista en `src/lib/security-headers.ts`, con test que pone rojo si alguien quita uno). CSP en **Report-Only** con receptor en `/api/csp-report` (rate-limited, sólo resumen a logs) — enforcement con nonces queda como follow-up cuando el inventario de violaciones esté recolectado. Sentry del navegador entra por el túnel `/monitoring`, así que `connect-src 'self'` basta. |
