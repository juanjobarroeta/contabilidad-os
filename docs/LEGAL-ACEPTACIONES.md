...
| e.firma cifrada que ya estaba almacenada (`GET/POST /api/companies/[id]/mandato-efirma`) | Autorización vigente por empresa | Flujo de sólo aceptación en Configuración; no descifra ni reemplaza la credencial. Sólo sesión web de `OWNER`, `ADMIN` o `ACCOUNTANT` con membresía real y acceso irrestricto/`CONTABILIDAD`; rechaza `VIEWER`, acceso sintético de soporte, bearer tokens y usuarios sólo verticales. Un usuario que también sea operador puede aceptar únicamente en su capacidad separada como miembro real del cliente. Aceptación y auditoría se insertan en una transacción. |
