...
Calculation callers resolve the `CompanyRegimen` rows whose known lifecycle overlaps the requested month or year. Once relation evidence exists it is authoritative; `Company.regimenFiscal` is used only for a legacy company with no relation rows, so today's scalar cannot silently replace a historical regime. Monthly planning may select one implemented track only when every companion is explicitly `NOT_APPLICABLE` monthly, such as business/professional activity (`612`) alongside salary, dividends, or interest (`605`, `611`, `614`); those sources do not share the monthly invoice basket. Two runnable monthly tracks, any assisted/unknown/incompatible track, every mixed annual, and a period with no matching relation row fail before fiscal reads with `422 NOT_SUPPORTED`. This prevents the primary engine from absorbing another regime's income; it is a safety boundary, not completed multi-regime composition. Explicit invoice/deduction/credit allocation and CSF-obligation-driven execution remain required.

### Invoice-to-regime assignment contract (FISC-002G)

`InvoiceRegimenAssignment` and its allocation rows store one accountant-reviewed attribution for an income or expense CFDI. `GET/PUT/DELETE /api/facturas/:id/asignacion-regimen` exposes the contract. A complete replacement must use only regimes whose lifecycle overlaps the invoice's UTC fiscal month and must total exactly 10,000 integer basis points (100.00%). Zero-share regimes are omitted. Every save records reviewer identity and time, increments an optimistic-concurrency revision, and emits append-only audit metadata; stale writes return `409 REVISION_CONFLICT` instead of overwriting another accountant's work.

This is ISR income/deduction attribution evidence, not a tax determination. It does not allocate IVA, choose a deduction's legal treatment, infer a regime from the CFDI use/customer/account, or change any calculation. `usadaEnCalculoAutomatico` therefore remains `false` in the API and the existing `422 NOT_SUPPORTED` mixed-regime boundary stays in force. A later composition slice must define payment-level application, deterministic rounding, credits/retentions, annual baskets, incomplete-data behavior, UI review states, and professional golden-case approval before consuming these rows.

The invoice detail is the first review surface. It stays out of the way for an ordinary single-regime CFDI with no stored assignment; a multi-regime month exposes exact percentage inputs, the 100.00% completeness gate, an optional note, reviewer evidence, and explicit loading/empty/stale/conflict/error/success/read-only states. Deleting a review returns the CFDI to `SIN_ASIGNAR`. This surface makes the evidence operable; it does not make the composition engine complete.

`GET /api/impuestos/asignaciones-regimen` adds the emission-month completeness view. It resolves the company's lifecycle evidence for the requested UTC month and, only when several recognized regimes overlap, evaluates every stamped income or expense CFDI as `COMPLETA`, `SIN_ASIGNAR`, or `REQUIERE_REVISION`. The response returns exact whole-period counts and at most 25 pending invoice previews; the Facturas screen shows the first six as direct review actions. Single-regime periods skip the invoice scan and render no extra queue. Missing or unknown regime evidence produces a warning instead of an empty/success state.

“Complete” in this queue means only that each candidate CFDI has valid emission-month attribution evidence. It does not mean a mixed-regime tax calculation is ready. PPD collections/payments still need payment-level application and deterministic rounding; IVA, deduction eligibility, retentions, credits, annual baskets, declaration state, and accounting close remain outside this contract. The endpoint therefore keeps `usadaEnCalculoAutomatico: false` even when its evidence count is complete.

### PPD payment projection boundary (FISC-002J)

`GET /api/impuestos/asignaciones-regimen/ppd` follows REP relations by their legal `FechaPago` month and resolves each parent CFDI by normalized UUID. The pure projection first derives the paid subtotal-equivalent base with integer-cent arithmetic. It then uses the largest-remainder method to apply the parent's reviewed basis points, assigning a residual cent by regime-code order so input order cannot change the result and every projected row sums exactly to the payment base.

A parent issued in a confirmed single-regime month gets an implicit 100% evidence split; no assignment row is fabricated. A parent from a multi-regime month must have a current valid review. Missing or ambiguous parents, missing amounts, stale assignments, absent/unknown lifecycle evidence, and non-PPD/non-stamped parents remain pending. Foreign-currency parents also remain pending because `PagoDoctoRelacionado` does not yet persist the REP's document currency/equivalence; applying the parent's raw amounts as MXN would be unsafe. If an assigned regime is absent in the payment month, the contract returns `REGIME_TRANSITION_REVIEW` instead of choosing whether old- or new-regime law applies.

This is still readiness evidence, not tax-engine output. The API returns exact relation counts and at most 25 pending previews, keeps `usadaEnCalculoAutomatico: false`, and does not touch IVA, deduction eligibility, retentions, credits, declarations, or accounting close. Connecting projected rows to regime-specific ISR baskets requires separate legal review, aggregation rules, golden cases, and explicit removal of the existing fail-closed calculation boundary.

The Facturas workspace consumes that contract as a pending-work surface, not as a result card. It stays hidden when a month has no PPD relation blockers, groups exact whole-period blocker counts, and shows at most six direct work items. When a parent CFDI exists, the accountant can open it from the payment-period queue and review its emission-period assignment; missing or ambiguous parents remain visible without pretending there is a corrective action. Saving or deleting an assignment refreshes both the emission and PPD queues. A complete or hidden queue still grants no calculation or closing permission.

Projection also needs evidence outside the selected payment month. For every current relation, the boundary reads all stamped REP relations tied to the normalized parent UUID and accumulates `ImpPagado` in integer millionths. Any missing or invalid historical amount leaves the current relation pending; a cumulative stamped amount above the parent total blocks every current projection for that parent. Draft and cancelled REP do not contribute. This guard detects inconsistent source evidence but does not decide how to repair it or authorize tax consumption.

The migration is additive and intentionally has no backfill: every existing CFDI begins `SIN_ASIGNAR`. If rollback is required, deploy code that does not reference the new models first, then drop the two new tables; no current calculation depends on them. Product/legal assumption recorded 2026-09-15 against the existing SAT/RMF 2026 baseline below: an accountant's attribution is provenance for a future workpaper, never proof that an amount is deductible, taxable, or creditable.
