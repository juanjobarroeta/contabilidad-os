...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | The fail-closed full-set boundary shipped in PR #1137 / `7bbd35c9`; the monthly planner in #1142 / `ddab03b0`; canonical CSF lifecycle in #1143 / `febcdd91`; primary-choice recovery in #1146 / `cd1fb8a7`; effective-period calculation in #1148 / `359bcd89`; historical annual evidence in #1150 / `b03bd684`; the reviewed invoice-allocation contract in #1151 / `62a20737`; its accountant review UI in #1153 / `6c990009`; and the emission-month completeness queue in #1155 / `4e791b63`. Calculation boundaries resolve lifecycle evidence for the requested month/year instead of blindly applying today's active set. A sole implemented monthly engine runs only when every companion is explicitly `NOT_APPLICABLE`; unsafe monthly sets and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED`. Allocation remains evidence only. Exit still requires PPD payment projection/consumption, deduction/credit consumption, authoritative CSF-obligation execution, and lawful annual composition. |
...
| FISC-002J PPD regime projection contract | IN_PROGRESS | Exact integer millionth-to-cent proration derives the subtotal-equivalent base of each REP relation, then a largest-remainder split applies reviewed basis points without losing or inventing a cent; regime-code ordering deterministically resolves ties. A single-regime invoice month derives 100% evidence without fabricating a stored review, while a multi-regime parent requires a valid assignment. Missing parents/amounts/regimes fail closed, duplicate normalized UUIDs are ambiguous, foreign-currency parents wait for stored REP equivalence, and any regime transition between emission and payment months stops for accountant review instead of deciding transition law. The read-only PPD readiness API counts every REP relation by `FechaPago`, bounds previews, and remains disconnected from IVA, declarations, closes, and automatic calculations. Focused suite: 2 files / 30 tests; full suite: 424 files / 4,621 tests; type checking and the 391-page production build pass. PR, CI, deployment, and authenticated smoke remain. |
