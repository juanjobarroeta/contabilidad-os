...
| REL-002 | Unified close state machine | IN_PROGRESS | REL-002A gives the guided close, owner view, AI context, daily pass, and monthly ZIP one fail-closed phase: blockers outrank submitted/posted/closed facts. Posting transitions and individual artifact routes still need the same guard before the item can exit |
...
| REL-002A Canonical close state and package gate | VERIFY | One pure resolver now orders `BLOQUEADO > CERRADO > POSTEADO > LISTO`; hard blockers, missing evidence, and changed confirmations override a submitted declaration or a physical ledger status. The close UI, owner summary, AI context, daily close cache, and monthly ZIP consume it; blocked or unposted ZIP requests return `409 CIERRE_NO_DESCARGABLE`. Focused suite: 6 files / 61 tests; full suite: 336 files / 3,687 tests; type checking and the 374-page production build pass. CI remains. Evidence: [`docs/cierre/estado-canonico.md`](./cierre/estado-canonico.md) |
