...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | The fail-closed full-set boundary shipped in PR #1137 / `7bbd35c9`; the monthly planner in #1142 / `ddab03b0`; canonical CSF lifecycle in #1143 / `febcdd91`; primary-choice recovery in #1146 / `cd1fb8a7`; effective-period calculation in #1148 / `359bcd89`; historical annual evidence in #1150 / `b03bd684`; the reviewed invoice-allocation contract in #1151 / `62a20737`; its accountant review UI in #1153 / `6c990009`; the emission-month completeness queue in #1155 / `4e791b63`; the PPD payment projection contract in #1156 / `e9f41361`; its pending-work UI in #1157 / `a2d3dc40`; and cumulative stamped-REP integrity in #1158 / `eeba246a`. Calculation boundaries resolve lifecycle evidence for the requested month/year instead of blindly applying today's active set. A sole implemented monthly engine runs only when every companion is explicitly `NOT_APPLICABLE`; unsafe monthly sets and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED`. Allocation remains evidence only. Exit still requires payment/deduction/credit consumption, authoritative CSF-obligation execution, and lawful annual composition. |
| OPS-001 | Health/readiness and Railway checks | DONE | PR #1172 / `332caf77` passed all five CI checks and shipped in exact Railway deployment `42c81e50`. Its resolved configuration targets `/api/ready` with a 120-second window and retains mandatory migrations; Railway logged the readiness healthcheck succeeding before promotion. Startup found 144 migrations with none pending and was ready in 761 ms. Deployment-attributed GET/HEAD health and ready return 200/no-store; login/assets and protected fiscal API smoke pass, with no initial 5xx rows. Focused suite: 27 tests; local full suite: 4,719 tests; real-Postgres suite: 31 tests. Disposable local production-mode tests prove database failure returns 503 without losing liveness, followed by recovery. No tax calculation, migration, or worker configuration changed. The separate seven-day Phase 0 observation remains open. Evidence: [`docs/operations/ops-001-readiness-2026-09-23.md`](./operations/ops-001-readiness-2026-09-23.md) |
...
| FISC-002H Invoice allocation review UI | VERIFY | The invoice detail loads the period-specific contract only for income/expense CFDIs and stays hidden for ordinary single-regime invoices without evidence to review. Multi-regime invoices expose exact two-decimal percentage inputs, a live 100.00% gate, optional review note, saved reviewer evidence, stale-evidence recovery, read-only access, retry/error/conflict states, and an explicit warning that the assignment does not split IVA or alter calculations. Removing evidence returns the invoice to `SIN_ASIGNAR`; the modal now scrolls within small screens. PR #1153 / `6c990009` passed all five CI checks and shipped through exact Railway deployment `d2b8ec8c`; startup found 142 migrations with none pending and the exact release was ready in 1.139 s. Public smoke returned `/login` `200`, `/facturas` `307` to login, and the protected allocation API `401`. Authenticated Altiplano smoke opened a stamped August income CFDI and correctly kept the control hidden for its single-regime month. Focused suite: 2 files / 25 tests; full suite: 420 files / 4,565 tests; type checking and the 389-page production build pass. A visible multi-regime production fixture remains unavailable; its states are covered automatically. |
| FISC-002I Period allocation evidence queue | VERIFY | A read-only monthly contract evaluates the effective lifecycle set and every stamped income/expense CFDI in the UTC emission month as complete, missing, or stale. The common single-regime path skips the invoice scan and stays out of the UI; multi-regime periods show bounded pending work plus exact whole-period counts. Missing/unknown regime evidence fails closed. The queue explicitly excludes PPD flow allocation, IVA, calculation changes, declarations, and closes. PR #1155 / `4e791b63` passed all five CI checks and shipped through exact Railway deployment `7dc65cdf`; startup found 142 migrations with none pending and the release was ready in 1.168 s. Public smoke returned `/login` `200`, `/facturas` `307` to login, and the protected period-queue API `401`. Focused suite: 2 files / 14 tests; full suite: 422 files / 4,591 tests; type checking and the 390-page production build pass. Authenticated Altiplano smoke remains unverified because the workstation locked again. |
| FISC-002J PPD regime projection contract | VERIFY | Exact integer millionth-to-cent proration derives the subtotal-equivalent base of each REP relation, then a largest-remainder split applies reviewed basis points without losing or inventing a cent; regime-code ordering deterministically resolves ties. A single-regime invoice month derives 100% evidence without fabricating a stored review, while a multi-regime parent requires a valid assignment. Missing parents/amounts/regimes fail closed, duplicate normalized UUIDs are ambiguous, foreign-currency parents wait for stored REP equivalence, and any regime transition between emission and payment months stops for accountant review instead of deciding transition law. The read-only PPD readiness API counts every REP relation by `FechaPago`, bounds previews, and remains disconnected from IVA, declarations, closes, and automatic calculations. PR #1156 / `e9f41361` passed all five CI checks and shipped through exact Railway deployment `8112f8f2`; startup found 142 migrations with none pending and the release was ready in 763 ms. Public smoke returned `/login` `200`, `/facturas` `307` to login, and the protected PPD readiness API `401`. Focused suite: 2 files / 30 tests; full suite: 424 files / 4,621 tests; type checking and the 391-page production build pass. Authenticated UI smoke remains unverified because the workstation is locked. |
| FISC-002K PPD readiness UI | VERIFY | The monthly Facturas workspace loads payment-period readiness independently from emission-period assignment readiness. It remains hidden when there are no PPD blockers, groups exact blocker counts, lists at most six direct work items, opens an available parent CFDI for assignment or review, and refreshes after assignment changes. Missing/ambiguous parents remain visible without a false action. The surface repeats that this is preparation evidence only and does not alter IVA, calculations, declarations, or closes. PR #1157 / `a2d3dc40` passed all five CI checks and shipped through exact Railway deployment `2f6a886b`; startup found 142 migrations with none pending and the release was ready in 637 ms. Public smoke returned `/login` `200`, `/facturas` `307` to login, and the protected PPD readiness API `401`. Focused suite: 3 files / 35 tests; full suite: 425 files / 4,626 tests; type checking and the 391-page production build pass. Authenticated UI smoke remains unverified because the workstation is locked. |
| FISC-002L Cumulative PPD integrity | VERIFY | Before projecting a current relation, the readiness boundary reads every stamped REP relation for the same normalized parent UUID across all periods. Missing, non-positive, or otherwise unsafe historical `ImpPagado` evidence blocks projection; the current relation must appear in that all-period snapshot; and exact BigInt accumulation blocks all current projections for a parent when stamped payments collectively exceed its total. Draft/cancelled REP remain outside the evidence set. The all-period lookup uses the existing `PagoDoctoRelacionado_parentUuid_idx`; no migration was needed. The UI exposes separate whole-period counts for incomplete history and cumulative overpayment, while automatic tax use remains disabled. PR #1158 / `eeba246a` passed all five CI checks and shipped through exact Railway deployment `c1891af8`; startup found 142 migrations with none pending and the release was ready in 650 ms. Public smoke returned `/login` `200`, `/facturas` `307` to login, and the protected PPD readiness API `401`. Focused suite: 3 files / 45 tests; full suite: 425 files / 4,636 tests; type checking and the 391-page production build pass. Authenticated UI smoke remains unverified because the workstation is locked. |
| SEC-DEP-001A Critical runtime advisories | DONE | Next.js, NextAuth, and the Auth Prisma adapter received bounded patch/beta updates, deduplicating Auth.js Core at 0.41.3. At this slice, the production audit moved from 4 critical / 15 high / 3 moderate to 0 critical / 15 high / 4 moderate, with 3,674 passing tests and a passing build. Final combined CI and production acceptance is recorded under SEC-DEP-001D. Historical evidence: [`docs/security/dependency-audit-2026-09-09.md`](./security/dependency-audit-2026-09-09.md) |
| SEC-DEP-001B Safe transitive patches | DONE | Ten transitive resolutions within existing direct-version ranges reduced the production audit from 15 to 8 high and from 4 to 2 moderate advisories. This slice passed clean install, all 3,681 tests, and the production build. The remaining direct/transitive findings were resolved by C/D without requiring Prisma 7 or Next.js 16. Final combined CI and production acceptance is recorded under SEC-DEP-001D. Historical evidence: [`docs/security/dependency-audit-2026-09-09.md`](./security/dependency-audit-2026-09-09.md) |
| SEC-DEP-001C SheetJS advisory remediation | DONE | `xlsx` moved from the stale npm-registry 0.18.5 release to SheetJS's official 0.20.3 tarball, clearing both known parser advisories without application-code changes. This slice passed clean install, 58 focused spreadsheet tests, all 3,677 tests, type checking, and the production build. Final combined CI and production acceptance is recorded under SEC-DEP-001D. Historical evidence: [`docs/security/dependency-audit-2026-09-09.md`](./security/dependency-audit-2026-09-09.md) |
| SEC-DEP-001D Remaining high dependency findings | DONE | Facturapi 5.1.0 removes Axios; Prisma/client 6.19.3 brings patched Effect; scoped overrides select deepmerge-ts 8.0.2 and PostCSS 8.5.28; Next.js 15.5.26 includes the September 22 security patch. Vendor SDK types, original-byte CSD uploads, stream/Blob downloads, and structured errors are covered by 3 focused files / 27 tests. Local full suite: 427 files / 4,669 tests; clean install and the 391-page build pass. PR #1162 / `3cc86929` passed all five CI checks and shipped in Railway deployment `8b3947fd`; startup found 142 migrations with none pending and was ready in 771 ms. Deployment-attributed public smoke passed login, CSS, redirects, and unauthenticated fiscal API boundaries; the initial HTTP error check returned no 5xx rows. No live PAC action or migration was needed. Evidence: [`docs/security/dependency-audit-2026-09-22.md`](./security/dependency-audit-2026-09-22.md) |
