...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | All calculation boundaries resolve `Company.regimenFiscal` together with every `CompanyRegimen`. Monthly planning uses the sole implemented engine only when every other active track is explicitly `NOT_APPLICABLE` monthly (for example 612 + 605/611/614); two runnable tracks, assisted/unknown/incompatible tracks, and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED` / `MULTI_REGIME_COMPOSITION_REQUIRED`. The initial boundary shipped in PR #1137 / `7bbd35c9` and Railway deployment `e077c18c`; production rendered the no-amount multi-regime state, while Altiplano monthly and annual single-regime paths stayed usable. Focused suite: 3 files / 51 tests; full suite: 412 files / 4,488 tests; type checking and the 389-page production build pass. Exit still requires explicit per-regime income/deduction allocation, CSF-obligation-driven execution, and lawful annual composition. |
