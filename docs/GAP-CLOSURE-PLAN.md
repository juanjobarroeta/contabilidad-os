...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | The fail-closed full-set boundary shipped in PR #1137 / `7bbd35c9`; the monthly planner in #1142 / `ddab03b0`; canonical CSF lifecycle in #1143 / `febcdd91`; primary-choice recovery in #1146 / `cd1fb8a7`; effective-period calculation in #1148 / `359bcd89`; historical annual evidence in #1150 / `b03bd684`; the reviewed invoice-allocation contract in #1151 / `62a20737`; its accountant review UI in #1153 / `6c990009`; the emission-month completeness queue in #1155 / `4e791b63`; the PPD payment projection contract in #1156 / `e9f41361`; and its pending-work UI in #1157 / `a2d3dc40`. Calculation boundaries resolve lifecycle evidence for the requested month/year instead of blindly applying today's active set. A sole implemented monthly engine runs only when every companion is explicitly `NOT_APPLICABLE`; unsafe monthly sets and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED`. Allocation remains evidence only. Exit still requires payment/deduction/credit consumption, authoritative CSF-obligation execution, and lawful annual composition. |
...
| FISC-002L Cumulative PPD integrity | IN_PROGRESS | Before projecting a current relation, the readiness boundary reads every stamped REP relation for the same normalized parent UUID across all periods. Missing, non-positive, or otherwise unsafe historical `ImpPagado` evidence blocks projection; the current relation must appear in that all-period snapshot; and exact BigInt accumulation blocks all current projections for a parent when stamped payments collectively exceed its total. Draft/cancelled REP remain outside the evidence set. The UI exposes separate whole-period counts for incomplete history and cumulative overpayment, while automatic tax use remains disabled. Focused suite: 3 files / 45 tests; full suite: 425 files / 4,636 tests; type checking and the 391-page production build pass. PR, CI, deployment, and authenticated smoke remain. |
