...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | Monthly and annual engine boundaries now resolve the normalized `Company.regimenFiscal` scalar together with every `CompanyRegimen` row. More than one distinct regime fails closed with stable `422 NOT_SUPPORTED` / `MULTI_REGIME_COMPOSITION_REQUIRED` before fiscal data is read, so the primary regime formula cannot consume mixed income. Focused suite: 3 files / 46 tests; full suite: 412 files / 4,474 tests; type checking and the 389-page production build pass. Exit still requires explicit per-regime income/deduction allocation, CSF-obligation-driven execution, and lawful annual composition. |
