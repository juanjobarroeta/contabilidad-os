...
| SEC-DEP-001 | Production dependency remediation | VERIFY | SEC-DEP-001D clears the remaining seven high findings: the root production audit now reports 0 critical, 0 high, and 1 moderate. Facturapi moves to its native-fetch SDK, Prisma and Next.js stay on supported major lines, and scoped transitive overrides carry compatibility evidence. CI now rejects high/critical production findings. Local compatibility, full-suite, and build checks pass; CI and production acceptance remain. Evidence: [`docs/security/dependency-audit-2026-09-22.md`](./security/dependency-audit-2026-09-22.md) |
...
| SEC-DEP-001D Remaining high dependency findings | VERIFY | Facturapi 5.1.0 removes Axios; Prisma/client 6.19.3 brings patched Effect; scoped overrides select deepmerge-ts 8.0.2 and PostCSS 8.5.28; Next.js 15.5.26 includes the September 22 security patch. The obsolete ambient SDK declaration is removed, uploads use original certificate bytes, downloads handle both streams and Blob, and native SDK errors retain structured diagnostics. Focused real-SDK/Prisma contracts: 3 files / 27 tests; full suite: 427 files / 4,669 tests; clean install and the 391-page production build pass. No migration is needed. CI and deployment evidence remain before closure. Evidence: [`docs/security/dependency-audit-2026-09-22.md`](./security/dependency-audit-2026-09-22.md) |
