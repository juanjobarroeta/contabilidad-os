...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | All calculation boundaries resolve `Company.regimenFiscal` together with every active `CompanyRegimen`. The fail-closed full-set boundary shipped in PR #1137 / `7bbd35c9`; the period-aware monthly planner shipped in PR #1142 / `ddab03b0` and Railway deployment `8ab713e4`. Monthly planning uses the sole implemented engine only when every companion track is explicitly `NOT_APPLICABLE` monthly (for example 612 + 605/611/614); two runnable tracks, assisted/unknown/incompatible tracks, and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED` / `MULTI_REGIME_COMPOSITION_REQUIRED`. Production rendered the no-amount multi-regime state, and Altiplano monthly and 2025 annual paths remained usable on the exact second deployment. The active CSF lifecycle slice recognizes all 19 catalog codes, synchronizes current/inactive regime rows without choosing the first PDF row as primary, and makes fiscal consumers ignore ended rows; focused suite: 6 files / 75 tests; full suite: 414 files / 4,502 tests; type checking and the 389-page production build pass. CI and deployment remain. Exit still requires explicit per-regime income/deduction allocation, authoritative CSF-obligation execution, and lawful annual composition. |
...
| FISC-002C CSF current-regime lifecycle | IN_PROGRESS | CSF parsing uses the canonical 19-code catalog and never infers the primary from the first row. A validated sync plan activates the current set, ends missing rows with provenance, preserves or explicitly requires the primary, and updates the scalar and relation atomically; fiscal calculations, declaration coverage, obligations, and credit inputs read active rows only. Focused suite: 6 files / 75 tests; full suite: 414 files / 4,502 tests; type checking and the 389-page production build pass. Migration drift, CI, and production deployment remain. Authoritative removal of obligations absent from a newer CSF is intentionally still FISC-003 scope. |
