...
| REL-002 | Unified close state machine | IN_PROGRESS | REL-002A gives the guided close, owner view, AI context, daily pass, and monthly ZIP one fail-closed phase for locally managed periods. REL-002B distinguishes historical declarations closed outside ContabilidadOS without inventing a local ledger or package. Monthly accounting transitions and individual artifact routes still need the same guard before the item can exit |
