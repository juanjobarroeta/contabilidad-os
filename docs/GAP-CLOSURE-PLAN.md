...
| REL-002 | Unified close state machine | IN_PROGRESS | REL-002A gives the guided close, owner view, AI context, daily pass, and monthly ZIP one fail-closed phase for locally managed periods. REL-002B distinguishes historical declarations closed outside ContabilidadOS without inventing a local ledger or package. REL-002C applies the same gate to every individual Anexo 24 download. REL-002D guards every monthly accounting transition at the engine boundary. Production smoke still remains before the item can exit |
