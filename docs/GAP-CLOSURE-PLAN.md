...
| FISC-002 | Multi-regime composition model | IN_PROGRESS | The fail-closed full-set boundary shipped in PR #1137 / `7bbd35c9`; the monthly planner shipped in PR #1142 / `ddab03b0`; canonical CSF lifecycle shipped in PR #1143 / `febcdd91`; accountant recovery for an ambiguous primary shipped in PR #1146 / `cd1fb8a7`; and effective-period calculation shipped in PR #1148 / `359bcd89`. Calculation boundaries and historical annual-evidence coverage resolve lifecycle evidence for the requested month/year instead of blindly applying today's active set. A sole implemented monthly engine runs only when every companion is explicitly `NOT_APPLICABLE`; unsafe monthly sets and every mixed annual fail before fiscal reads with `422 NOT_SUPPORTED`. Exit still requires explicit per-regime income/deduction allocation, authoritative CSF-obligation execution, and lawful annual composition. |
...
| FISC-002G Accountant-reviewed invoice allocation contract | IN_PROGRESS | Additive assignment/header and per-regime rows represent a complete income/expense CFDI split in exact integer basis points. The API restricts codes to lifecycle evidence for the invoice month, stamps the reviewer, uses optimistic revisions, blocks VIEWER writes, records append-only audit metadata, and reports stale stored evidence for re-review. No backfill or calculation behavior changes: mixed-regime math remains fail-closed, and IVA/annual/payment-level composition are explicitly outside this slice. PR #1151; focused suite: 2 files / 22 tests; full suite: 419 files / 4,550 tests; type checking, migration drift, and the 389-page production build pass. CI, deployment, and production smoke remain. |
