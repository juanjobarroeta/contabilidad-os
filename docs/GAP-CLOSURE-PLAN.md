...
| FISC-002K PPD readiness UI | IN_PROGRESS | The monthly Facturas workspace loads payment-period readiness independently from emission-period assignment readiness. It remains hidden when there are no PPD blockers, groups exact blocker counts, lists at most six direct work items, opens an available parent CFDI for assignment or review, and refreshes after assignment changes. Missing/ambiguous parents remain visible without a false action. The surface repeats that this is preparation evidence only and does not alter IVA, calculations, declarations, or closes. Focused suite: 3 files / 35 tests; full suite: 425 files / 4,626 tests; type checking and the 391-page production build pass. PR, CI, deployment, and authenticated smoke remain. |
