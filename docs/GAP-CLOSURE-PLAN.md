...
| REL-002 | Unified close state machine | VERIFY | REL-002A-D are merged and deployed. Production evidence confirms that a historical declaration stays closed outside ContabilidadOS without a local ledger, while a locally managed `POSTED` period with new bank blockers returns to `BLOQUEADO` and cannot generate its package. Final acceptance requires daily-pass monitoring, a controlled mutating close/reopen smoke in a disposable company, and licensed-accountant sign-off |
