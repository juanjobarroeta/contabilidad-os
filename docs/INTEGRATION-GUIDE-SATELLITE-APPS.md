...
- **Cuenta puente, never the real bank account.** Pushed income goes to a **dedicated clearing account** (e.g. tipo CAJA) that never receives statement uploads. Pushing into the statement-fed account would double-count every charge — and the duplicates aren't even detectable, because Stripe payouts land batched, net of fees, days later. CFDIs are reconciled against the per-invoice credits in the clearing account; the payout line on the real statement is categorized as a traspaso, and Stripe fees as a periodic gasto. This is **enforced server-side**: `POST /api/ingest/external-income` rejects (409) accounts fed by uploads/Belvo, and statement imports reject accounts fed by external ingest (`src/lib/bancos/fuentes.ts`).
