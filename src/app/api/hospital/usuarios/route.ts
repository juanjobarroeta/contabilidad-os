...
// GET /api/hospital/usuarios?companyId=xxx
// Miembros de la empresa con su rol y las PÁGINAS del satélite que pueden ver
// (la rejilla «cada quien ve lo que su trabajo requiere»: enfermería no ve
// nómina, caja no edita el expediente). Sólo OWNER/ADMIN. Mismo contrato que
// automotriz/usuarios sobre CompanyMember.hospitalPaginas.
...
// paso (mismo patrón que automotriz/usuarios):
//   - User con subscriptionStatus ACTIVE (los empleados no llevan trial).
//   - CompanyMember encajonado al módulo HOSPITAL (no puede entrar a la UI de
//     contabilidadOS) y a las páginas elegidas en la rejilla.
...
export const POST = withAuthz(async (req: Request) => {
  ...
  registrarBitacora({
    ...
    entidadId: member.id,
    detalle: { email, role, paginas, origen: "hospital" },
    req,
  });
