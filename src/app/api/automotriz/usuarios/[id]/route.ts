...
async function cargarContexto(id: string, req: Request) {
  ...
  await requireModule(target.companyId, "AUTOMOTRIZ", req);
...
// PATCH /api/automotriz/usuarios/[id] — id = membershipId.
  ...
  switch (data.action) {
    case "permisos": {
      ...
      const updated = await prisma.companyMember.update({
        where: { id },
        data: { role: data.role, automotrizPaginas: data.paginas },
      });
      registrarBitacora({
        ...
        detalle: {
          ...
          rol: updated.role,
          paginas: updated.automotrizPaginas,
          origen: "automotriz",
        },
      });
      return NextResponse.json({
        ...
        role: updated.role,
        paginas: updated.automotrizPaginas,
        sinRestriccion: updated.automotrizPaginas.length === 0,
      });
    }
...
// DELETE /api/automotriz/usuarios/[id] — quita el acceso a la empresa.
...
export const DELETE = withAuthz(async (req: Request, ctx: Params) => {
  ...
  registrarBitacora({
    ...
    entidad: "CompanyMember",
    entidadId: id,
    detalle: { email: target.user.email, origen: "automotriz" },
  });
  return NextResponse.json({ ok: true });
});
