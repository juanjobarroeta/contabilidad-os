...
// Una fila de captura = un PDF a subir. Las mensuales (ISR provisional + IVA) del
// mismo periodo se presentan en UN solo acuse, así que se agrupan en una fila y
// un upload guarda ambas. Las anuales van una por ejercicio.
interface Item {
  key: string;
  companyId: string;
  periodo: string;
...
function agrupar(companyId: string, faltantes: Faltante[]): Item[] {
  ...
  // Anuales primero (más antiguo primero); luego mensuales recientes primero.
...
export default function DeclaracionesPage() {
  ...
  async function subir(item: Item, file: File) {
    ...
    try {
      // 1. Parseo con el extractor existente (Claude Vision) — UNA vez.
      ...
      // 2. Guardar cada obligación de la fila desde el MISMO acuse/PDF (la
      //    mensual cubre ISR provisional + IVA en un solo comprobante).
