...
export default function EmpresaEditPage() {
  ...
  if (!company) {
    return (
      <div className="p-6">
        ...
        <Link href="/empresa" className="text-primary text-sm hover:underline mt-2 inline-block">
          ← Volver
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {/* ── Datos Generales ── */}
      <section className="bg-white border border-border rounded-xl shadow-sm p-5 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              maxLength={5} className="w-full px-3 py-2 border border-border rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
          <div className="sm:col-span-2">
        ...
        <button
          ...
          className="mt-4 flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-md text-sm font-medium hover:bg-primary/90 disabled:opacity-50"
        >
      ...
      <section className="bg-white border border-border rounded-xl shadow-sm p-5 mb-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${hasCsd ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
        ...
        <div className="space-y-3">
          ...
          {csdSuccess && <p className="text-xs text-green-700">{csdSuccess}</p>}
          <button
      ...
      <section className="bg-white border border-border rounded-xl shadow-sm p-5 mb-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium mb-1">Certificado <code>.cer</code></label>
            <label className="flex items-center gap-2 px-3 py-2.5 border border-dashed border-border rounded-md text-xs cursor-pointer hover:bg-gray-50">
              <Upload className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
              <span className="text-muted-foreground truncate">{fielCerFile ? fielCerFile.name : "Seleccionar .cer"}</span>
              <input type="file" accept=".cer" className="hidden"
          ...
          <div>
            ...
            <label className="flex items-center gap-2 px-3 py-2.5 border border-dashed border-border rounded-md text-xs cursor-pointer hover:bg-gray-50">
              <Upload className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
              <span className="text-muted-foreground truncate">{fielKeyFile ? fielKeyFile.name : "Seleccionar .key"}</span>
              <input type="file" accept=".key" className="hidden"
        ...
        {fielSuccess && <p className="text-xs text-green-700 mt-2">{fielSuccess}</p>}
        <button
          onClick={handleFielUpload}
          disabled={fielSaving || !fielCerFile || !fielKeyFile || !fielPassword}
          className="mt-3 flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-md text-sm font-medium hover:bg-primary/90 disabled:opacity-50"
        >
