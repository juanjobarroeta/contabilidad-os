...
// es por sustitución (CFF 29-A). Emite un hallazgo por cada par REP↔factura.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Un hallazgo por cada par REP↔factura con fecha de pago anterior a la emisión. */
export function auditarRepFechaPagoAnterior(items: RepFechaAnterior[]): Hallazgo[] {
  return items.map((i) => ({
    ...
    severidad: "warn",
    mensaje: `El complemento de pago (REP) ${i.repLabel} registra una fecha de pago (${i.fechaPago}) ANTERIOR a la emisión de la factura ${i.parentLabel} (${i.facturaFecha}) que liquida. Una FechaPago previa a la factura es inválida y, como el IVA se causa al cobro, manda el IVA al mes equivocado.`,
    referencias: [i.repId, i.parentId],
    fundamento: { ley: "CFF", articulo: "29-A" },
    sugerencia:
      "Corrige el comprobante por sustitución: lo más común es que el cobro fue antes de emitir la factura, así que debió ser PUE con la fecha/forma de pago real. Cancela con motivo 01 (con relación) y reexpide el sustituto correcto. Verifica además en qué mes quedó el IVA: debe coincidir con la fecha de cobro real.",
  }));
}
