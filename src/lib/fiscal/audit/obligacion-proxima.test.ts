...
describe("auditarObligacionProxima", () => {
  it("dentro de la ventana (≤7 días antes, sin presentar) → warn", () => {
    ...
    const out = auditarObligacionProxima(data(), fechaMx(2026, 6, 12));
  ...
  it("el mismo día del vencimiento (17, sin presentar) → warn (aún a tiempo)", () => {
    ...
    const out = auditarObligacionProxima(data(), fechaMx(2026, 6, 17));
    expect(out).toHaveLength(1);
  ...
  it("pasado el vencimiento (sin presentar) → error", () => {
    ...
    const out = auditarObligacionProxima(data(), fechaMx(2026, 6, 20));
    expect(out).toHaveLength(1);
    const h = out[0];
  ...
  it("lejos del vencimiento (>7 días antes) → sin hallazgo", () => {
    ...
    const out = auditarObligacionProxima(data(), fechaMx(2026, 6, 1));
    expect(out).toEqual([]);
  });

  it("cruce de año: enero mira a diciembre y respeta el fin de semana", () => {
    // Jan 17, 2026 is Saturday, so the shared service moves the deadline to 19.
    const out = auditarObligacionProxima(data(), fechaMx(2026, 0, 16));
    expect(out).toHaveLength(1);
    expect(out[0].severidad).toBe("warn");
    expect(out[0].referencias).toEqual(["IVA_MENSUAL:2025-12"]);
    expect(out[0].mensaje).toContain("diciembre");
    expect(out[0].mensaje).toContain("vence el 19");
  });
});
