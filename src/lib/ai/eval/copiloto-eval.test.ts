...
describe("PREGUNTAS_EVAL", () => {
  it("ids únicos y fundamentos en el formato de buildCita", () => {
    ...
    for (const p of PREGUNTAS_EVAL) {
      ...
      for (const f of p.fundamentos) expect(f).toMatch(/^(Art\. \S+( Bis)? (RLISR|RLIVA|RCFF|CCOM|LGSM|RLFPIORPI|LFPIORPI|LFDC|RACERF|RIPAEDI|LHPUE|CFPUE|CFCDMX|LISR|LIVA|CFF|LIEPS|LSS|LINFONAVIT|LFT)|Regla \d+(\.\d+)+ RMF-\d{4})$/);
    }
  });
});
